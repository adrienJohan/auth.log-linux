...
void signin_form () 
{
	printf("<div>\n"
           ...
           "\t\t<tr>\n"
           "\t\t\t<td>\n"
           "\t\t\t<form action =http://www.sitemanager.com/cgi-bin/pass.cgi method = get >\n" 
...
void redirection_form(char* user)
{
	printf("<div>\n"
    "\t<p> Bienvenue %s</p>\n"
    "\t<a href=http://www.sitemanager.com/main_page.html>Entrer dans le site</a>\n"
    "</div>\n", user);
}
...
void password_error()
{
	...
	"\t<a href=http://www.sitemanager.com/cgi-bin/login.cgi>retour à l'acceuil</a>\n"
	"</div>");
}
